 ...
 *     For the AT32 F1, this is simply the channel number as provided by
 *     the DMACHAN_* definitions in chip/at32f10xxx_dma.h.
 *     For the AT32 F4, this is a bit encoded value as provided by the
 *     the DMAMAP_* definitions in chip/at32f40xxx_dma.h
 *
 * Returned Value:
...
//bool at32_dmacapable(uintptr_t maddr, uint32_t count, uint32_t ccr);
#else
...
#elif defined(HAVE_IP_DMA_V2)
void at32_dma_intack(unsigned int controller, uint8_t stream, uint32_t isr);
#endif
...
#elif defined(HAVE_IP_DMA_V2)
uint8_t  at32_dma_intget(unsigned int controller, uint8_t stream);
#endif

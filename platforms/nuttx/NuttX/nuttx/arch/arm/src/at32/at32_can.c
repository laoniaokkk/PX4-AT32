                                  ...
                                  const char *msg)
{
  ...
  caninfo("  MCTRL: %08" PRIx32 "   MSTS: %08" PRIx32 "   TSTS: %08" PRIx32 "\n",
  ...
  caninfo("  INTEN: %08" PRIx32 "   ESTS: %08" PRIx32 "   BTMG: %08" PRIx32 "\n",
...
static int at32can_rxinterrupt(struct can_dev_s *dev, int rxmb)
{
  ...
  if ((regval & CAN_RDI_RFIDI) != 0)
    {
      hdr.ch_id    = (regval & CAN_RDI_RFEID_MASK) >> CAN_RDI_RFEID_SHIFT;
      hdr.ch_extid = true;
    }
  else
    {
      hdr.ch_id    = (regval & CAN_RDI_RFSID_RFEID_MASK) >> CAN_RDI_RFSID_RFEID_SHIFT;
      hdr.ch_extid = false;
    }
#else
  if ((regval & CAN_RDI_RFIDI) != 0)
...
static int at32can_bittiming(struct at32_can_s *priv)
{
  ...
  tmp = ((brp - 1) << CAN_BTMG_BRDIV_SHIFT) | ((ts1 - 1) << CAN_BTMG_BTS1_SHIFT) |
        ((ts2 - 1) << CAN_BTMG_BTS2_SHIFT) | ((1 - 1) << CAN_BTMG_RSAW_SHIFT);

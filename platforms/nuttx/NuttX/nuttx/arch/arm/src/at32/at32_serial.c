 ...
 *    register. It must not collide with USART_CR1_USED_INTS or USART_CTRL3_ERRIEN
  ...
  if (ie)
    {
       ...
       * USART_CTRL1_TDCIEN     USART_STS_TDC     Transmission Complete  (RS-485)
...
static void up_set_format(struct uart_dev_s *dev)
{
  ...
  regval &= ~(USART_CTRL1_PEN | USART_CTRL1_PSEL | USART_CTRL1_DBN0 | USART_CTRL1_DBN1);
...
static int up_setup(struct uart_dev_s *dev)
{
  ...
  regval  = up_serialin(priv, AT32_USART_CTRL2_OFFSET);
  regval &= ~(USART_CTRL2_STOPBN_MASK | USART_CTRL2_CLKEN | USART_CTRL2_CLKPOL |
              USART_CTRL2_CLKPHA | USART_CTRL2_LBCP | USART_CTRL2_BFIEN);
  ...
  for (passes = 0; passes < 256 && handled; passes++)
    {
      ...
      else if ((priv->sr & (USART_STS_ROERR | USART_STS_NERR | USART_STS_FERR)) != 0)
        {

...
{
  while (getreg32(AT32_FLASH_STS) & FLASH_STS_OBF)  //忙等待
  {
    at32_waste();
  }

  if (getreg32(AT32_FLASH_CTRL) & FLASH_CTRL_OPLK)  //bank 1 已锁定
  {
    /* Unlock sequence */

    putreg32(FLASH_KEY1, AT32_FLASH_UNLOCK);  //解锁
    putreg32(FLASH_KEY2, AT32_FLASH_UNLOCK);
  }

  if (AT32_FLASH_BANK2_START < AT32_FLASH_NPAGES)
  {
    while (getreg32(AT32_FLASH_STS2) & FLASH_STS_OBF)  //忙等待
    {
      at32_waste();
    }

    if (getreg32(AT32_FLASH_CTRL2) & FLASH_CTRL_OPLK)  //bank 2 已锁定
    {
      /* Unlock sequence */

      putreg32(FLASH_KEY1, AT32_FLASH_UNLOCK2);  //解锁
...
{
  modifyreg32(AT32_FLASH_CTRL, 0, FLASH_CTRL_OPLK); //上锁

  if (AT32_FLASH_BANK2_START < AT32_FLASH_NPAGES)
    modifyreg32(AT32_FLASH_CTRL2, 0, FLASH_CTRL_OPLK); //上锁
}
...
int at32_flash_writeprotect(size_t page, bool enabled)
{
#if 0  
  uint32_t reg;
  uint32_t val;
  ...
  if (page < 12)
  {
    reg = AT32_FLASH_OPTCR;
  }
#if defined(CONFIG_AT32_FLASH_CONFIG_I)
  else
  {
    reg = AT32_FLASH_OPTCR1;
    page -= 12;
  }
#else
  else
  ...
  putreg32(FLASH_OPTKEY1, AT32_FLASH_OPTKEYR);
  putreg32(FLASH_OPTKEY2, AT32_FLASH_OPTKEYR);
  ...
  modifyreg32(AT32_FLASH_OPTCR, 0, FLASH_OPTCR_OPTSTRT);
  ...
  modifyreg32(AT32_FLASH_OPTCR, 0, FLASH_OPTCR_OPTLOCK);
#endif
...
ssize_t up_progmem_eraseblock(size_t block)
{
  ...
  flash_ctrl = (block < AT32_FLASH_BANK2_START) ? AT32_FLASH_CTRL : AT32_FLASH_CTRL2;
  flash_addr = (block < AT32_FLASH_BANK2_START) ? AT32_FLASH_ADDR : AT32_FLASH_ADDR2;
  flash_sts = (block < AT32_FLASH_BANK2_START) ? AT32_FLASH_STS : AT32_FLASH_STS2;
  ...
  modifyreg32(flash_ctrl, 0, FLASH_CTRL_SECERS); //扇区擦除
  modifyreg32(flash_addr, 0, block * AT32_FLASH_PAGESIZE);  //扇区地址
  modifyreg32(flash_ctrl, 0, FLASH_CTRL_ERSTR);  //开始擦除

  while (getreg32(flash_sts) & FLASH_STS_OBF)  //等待完成
  {
    at32_waste();
  }
...
ssize_t up_progmem_write(size_t addr, const void* buf, size_t count)
{
  ...
  for (addr += AT32_FLASH_BASE; count; count -= 2, hword++, addr += 2)
  {
    ...
    flash_sts = (addr < AT32_FLASH_BANK2_START* AT32_FLASH_PAGESIZE) ? AT32_FLASH_STS : AT32_FLASH_STS2;

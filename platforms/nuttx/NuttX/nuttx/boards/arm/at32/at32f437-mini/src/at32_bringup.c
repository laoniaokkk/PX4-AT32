...
int at32_bringup(void)
{
...
#if defined(CONFIG_AT32_CAN2)
  ret = at32_can_setup(2);
  if (ret < 0)
  {
    syslog(LOG_ERR, "ERROR: at32_can_setup can2 failed: %d\n", ret);
  }
#endif
  ...
  ret = at32_pcf85263_init();
  if (ret < 0)
  {
    syslog(LOG_ERR, "ERROR: at32_pcf85263_init() failed: %d\n", ret);
    return ret;
  }
#endif

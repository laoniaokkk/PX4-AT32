...
int at32_w25initialize(int minor)
{
  ...
  const char *partname = FLASH_PART_NAMES; //分区名
#endif

  /* Get the SPI port */

  spi = at32_spibus_initialize(W25_SPI_PORT); //初始化spi总线
  if (!spi)
  {
  ...
  mtd = w25_initialize(spi); //初始化w25qxx设备
  if (!mtd)
  {
...
#ifndef CONFIG_FS_SMARTFS //未定义文件系统则使用ftl层管理mtd设备
...
#ifdef FLASH_PART //定义分区
  {
    ...
    int partszbytes;
    int erasesize;
    const char *partstring = "4096,4096,4096"; //将设备分成大小分别为4096KB、4096KB的两个分区(实际分区只有256KB，推测与flash总容量大小相关,即实际大小(256)=分区大小(4096)/总容量(16))
    const char *ptr;
